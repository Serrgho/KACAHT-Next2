...
Namespace Kas
	...
	Partial Public Class KasCentralConnectionIndicator
        ...
        ' Инициализация из MainWindow
        Public Async Sub Initialize()
            _fetcherClient = CentralFetcher.HttpClient
            ' Подписываемся на событие глобального CentralFetcher
            AddHandler CentralFetcher.CentralConnectionChanged, Async Sub() Await CheckNow()
        ...
        Public Async Function CheckNow() As Task
            ...
            _isChecking = True
            Try
                ' Используем HttpClient из глобального CentralFetcher
                Dim checkUrl As String = $"{CentralFetcher._baseUrl}/index.jsp"
                ...
                ' Проверка маркеров для центральной сессии
                Dim isLogged As Boolean = response.IsSuccessStatusCode AndAlso
                                 Not html.Contains("anauth_panel") AndAlso
                                 (html.Contains("Журналы") Or html.Contains("Смена") Or html.Contains("Отчёты"))
